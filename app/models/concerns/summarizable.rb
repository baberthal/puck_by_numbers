module Summarizable
  ...
  module ClassMethods
    def sitfind(id)
      SituationQuery.new(self).sitfind(id)
    end
  end

  def create_team_summary(team, situation, options={})
    team_params = { event_team_id: team.id }

    goals_query = goals.sitfind(situation).count
    ...
    toi_query = (events.where(skater_params).where.not(goalie_not_params).sum(:event_length)/60).round(1)
